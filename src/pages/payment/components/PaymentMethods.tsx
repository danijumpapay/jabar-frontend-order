...
export const PaymentMethods = ({ selectedMethod, onSelect }: PaymentMethodsProps) => {
  const paymentMethods: PaymentMethod[] = [
    ...
    { id: 'BCA', name: 'VA BCA', logo: '/payments/bca.svg' },
        ...
        {paymentMethods.map((method) => {
          ...
          return (
            <div
              ...
              className={`p-4 md:p-5 border rounded-2xl flex items-center gap-3 md:gap-4 cursor-pointer transition-all ${isSelected
                ? 'border-[#27AAE1] bg-[#F0F9FF]'
                : 'border-gray-100 hover:border-gray-200 bg-white'
                }`}
            >
              <div className={`w-5 h-5 shrink-0 rounded-full border flex items-center justify-center transition-all ${isSelected ? 'border-[#27AAE1] bg-white' : 'border-gray-300'
                }`}>
              ...
              <div className="flex flex-1 items-center justify-between gap-2">
                ...
                <span className={`text-[10px] md:text-[11px] font-bold text-right leading-tight ${isSelected ? 'text-[#27AAE1]' : 'text-gray-700'
                  }`}>
                  {method.name}
                </span>
              </div>
